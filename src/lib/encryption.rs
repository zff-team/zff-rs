...
use pkcs5::{
    EncryptionScheme, pbes2::Parameters as PBES2Parameters, scrypt::Params as ScryptParams,
};
use rand::Rng;
#[cfg(feature = "serde")]
...
pub fn encrypt_scrypt_aes128cbc(
    ...
    let params =
        PBES2Parameters::generate_scrypt_aes128cbc(ScryptParams::new(logn, r, p)?, salt, aes_iv)?;
    let encryption_scheme = EncryptionScheme::Pbes2(params);
...
pub fn encrypt_scrypt_aes256cbc(
    ...
    let params =
        PBES2Parameters::generate_scrypt_aes256cbc(ScryptParams::new(logn, r, p)?, salt, aes_iv)?;
    let encryption_scheme = EncryptionScheme::Pbes2(params);
    Ok(encryption_scheme.encrypt(password, plaintext)?)
}
...
pub fn encrypt_argon2_aes128cbc(
    ...
    encrypt_argon2_aes(
        password, salt, mem_cost, lanes, iterations, scheme, aes_iv, plaintext,
    )
}
    ...
    P: AsRef<[u8]>,
{
    let hash = match scheme {
        PBEScheme::AES128CBC => {
            hash_password_argon2(password, salt, mem_cost, lanes, iterations, 16)?
        }
        PBEScheme::AES256CBC => {
            hash_password_argon2(password, salt, mem_cost, lanes, iterations, 32)?
        }
    };
