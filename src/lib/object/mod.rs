...
use crate::prelude::*;
use crate::{
    EncryptionAlgorithm, PreparedChunk,
    helper::zstd_compress_if_worthwhile,
    io::{calculate_xxhash, check_same_byte},
};

// - external
#[cfg(any(feature = "los_tar", feature = "vos_tar"))]
use {base64::Engine, base64::engine::general_purpose::STANDARD as base64engine, tar::Entry};
...
impl HashingThreadManager {
    ...
    pub fn add_thread(&mut self, hash_type: HashType) {
        if !self.threads.contains_key(&hash_type) {
            self.threads.insert(
                hash_type.clone(),
                HashingThread::new(hash_type, self.data.clone()),
            );
        }
    }
...
impl HashingThread {
    ...
    pub fn new(hash_type: HashType, data: Arc<RwLock<Vec<u8>>>) -> Self {
        let (trigger, receiver) =
            crossbeam::channel::unbounded::<(crossbeam::sync::WaitGroup, bool)>();
...
impl DeduplicationThread {
    ...
    pub fn new(data: Arc<RwLock<Vec<u8>>>) -> Self {
        let (trigger, trigger_receiver) =
            crossbeam::channel::unbounded::<crossbeam::sync::WaitGroup>();
        ...
        && *encoding_thread_pool_manager.same_bytes_thread.get_result()
    {
            ...
            .get_chunk_number(xxhash)
        {
            ...
            for chunk_no in chunk_no_set {
                let verification_hash = match deduplication_metadata
                    .deduplication_map
                    .get_verification_hash(chunk_no)?
                {
                    Some(vhash) => vhash,
                    None => {
                        ...
                        deduplication_metadata
                            .deduplication_map
                            .append_verification_hash(chunk_no, verification_hash)?;
                        verification_hash
                    }
                };
...
fn check_root_path<P: AsRef<Path>>(path: P) -> bool {
    ...
    let count = if path.as_ref().has_root() { 2 } else { 1 };
    path.as_ref().components().count() == count
}
...
#[cfg(any(feature = "los_tar", feature = "vos_tar"))]
fn get_metadata_ext_tar<R: Read>(
    entry: &mut Entry<R>,
        ...
        for ext_entry in pax_extensions {
            ...
            if let Ok(key) = ext_entry.key() {
                ...
                let ext_value = if let Ok(value) = ext_entry.value() {
                    match key.trim() {
                        METADATA_ATIME => MetadataExtendedValue::U64(
                            parse_unix_timestamp_nanos(value).unwrap_or(0),
                        ),
                        METADATA_MTIME => MetadataExtendedValue::U64(
                            parse_unix_timestamp_nanos(value).unwrap_or(0),
                        ),
                        METADATA_CTIME => MetadataExtendedValue::U64(
                            parse_unix_timestamp_nanos(value).unwrap_or(0),
                        ),
                        METADATA_BTIME => MetadataExtendedValue::U64(
                            parse_unix_timestamp_nanos(value).unwrap_or(0),
                        ),
                ...
                metadata_ext.insert(key, ext_value);
            }
        }
    }
