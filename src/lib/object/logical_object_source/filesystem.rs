    ...
    pub fn new<P: AsRef<Path>>(input_files: Vec<P>) -> Result<Self> {
        ...
        for path in input_files {
            ...
            if metadata.file_type().is_dir() {
                directories_to_traversal.push_back((
                ...
                if metadata.file_type().is_symlink() {
                    ...
                    match read_link(&path) {
                        Ok(symlink_real) => {
                            symlink_real_paths.insert(current_file_number, symlink_real)
                        }
                        Err(_) => symlink_real_paths.insert(current_file_number, PathBuf::from("")),
                    };
                }
                let mut file_header =
                    match get_file_header(path.as_ref(), current_file_number, parent_file_number) {
        ...
        while let Some((current_dir, dir_parent_file_number, dir_current_file_number)) =
            directories_to_traversal.pop_front()
        {
            ...
            for inner_element in element_iterator {
                ...
                let inner_element = match inner_element {
                    ...
                    Err(e) => {
                        ...
                        #[cfg(feature = "log")]
                        debug!(
                            "Error while trying to unwrap the inner element of the element iterator: {e}."
                        );
                        continue;
                    }
                };
                ...
                if metadata.file_type().is_dir() {
                    ...
                    let mut file_header =
                        match get_file_header(&path, current_file_number, parent_file_number) {
                            Ok(file_header) => file_header,
                            Err(e) => {
                                #[cfg(feature = "log")]
...
fn gen_filetype_encoding_information(
    filesystem_data: &LogicalObjectSourceFilesystem,
    ...
    match current_file_header.file_type {
        ...
        FileType::Hardlink => {
            let hardlink_filenumber = filesystem_data
                .hardlink_map
                .get(&current_file_number)
                .unwrap_or(&0);
