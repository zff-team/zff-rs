...
// - internal
use crate::prelude::*;
use crate::{EncodingState, EncodingThreadPoolManager, FileEncoder, PreparedData, Signature};
...
impl LogicalObjectEncoder {
    ...
    pub fn get_encoded_footer(&mut self) -> Result<Vec<u8>> {
        self.object_footer
            .replace_root_dir_filenumbers(self.logical_object_source.root_dir_filenumbers());
    ...
    pub fn new(
        ...
        let (filetype_encoding_information, file_header) = match logical_object_source.next() {
            Some(res) => res?,
            None => return Err(ZffError::new(ZffErrorKind::Missing, ERROR_NO_INPUT_FILE)),
        };
            ...
            Some(ref mut file_encoder) => {
                ...
                if !self.current_file_header_read {
                    ...
                    if self.object_footer.acquisition_start == 0 {
                        self.object_footer.acquisition_start =
                            OffsetDateTime::from(SystemTime::now()).unix_timestamp() as u64;
                    }
                    self.object_footer
                        .file_header_segment_numbers
                        .insert(self.current_file_number, current_segment_no);
                    self.object_footer
                        .file_header_offsets
                ...
                let prepared_file_footer =
                    PreparedData::PreparedFileFooter(file_encoder.get_encoded_footer()?);
                ...
                let (filetype_encoding_information, current_file_header) =
                    match self.logical_object_source.next() {
                        Some(res) => res?,
                        None => {
                            ...
                            self.object_footer.acquisition_end =
                                OffsetDateTime::from(SystemTime::now()).unix_timestamp() as u64;
