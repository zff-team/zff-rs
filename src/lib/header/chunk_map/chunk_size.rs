...
impl ChunkMap for ChunkSizeMap {
	type Value = u64;
	...
	fn current_size(&self) -> usize {
		match self.chunkmap.first_key_value() {
			Some(_) => self.chunkmap.len() * (8 + 8) + 8, //8 -> 8bytes for the chunk no, 8 bytes for size
			None => return 0,
		}
	}
	...
	fn is_full(&self) -> bool {
		if self.target_size < self.current_size() + 24 { //24 -> 8bytes for next chunk_no, 8 bytes for size, 8 bytes for the size of the encoded BTreeMap
			true
		} else {
			false
		}
