...
impl ChunkMaps {
    /// Generates a new map
    pub fn new(
        header_map: ChunkHeaderMap,
        same_bytes_map: ChunkSamebytesMap,
        duplicate_chunks: ChunkDeduplicationMap,
    ) -> Self {
        Self {
            header_map,
            same_bytes_map,
        ...
        S: Serializer,
    {
        ...
        if !self.same_bytes_map.is_empty() {
            state.serialize_field(
                string_to_str(self.same_bytes_map.to_string()),
                &self.same_bytes_map,
            )?;
        }
...
pub trait ChunkMap {
    ...
    fn decrypt_and_decode_at<K, A, D>(
        key: K,
        ...
        Self: HeaderCoding,
    {
        ...
        let encoded_header_length = (DEFAULT_LENGTH_HEADER_IDENTIFIER
            ...
            + encrypted_map.len()) as u64; //4 bytes identifier + 8 bytes for length + length itself
