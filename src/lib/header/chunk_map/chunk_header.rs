...
impl ChunkMap for ChunkHeaderMap {
    ...
    fn is_full(&self) -> bool {
        if self.target_size < self.current_size() + 54 {
            //54 -> 8bytes for next chunk_no, 38 bytes for the encoded chunk header, 8 bytes for the size of the encoded BTreeMap
            true
        } else {
            false
        }
