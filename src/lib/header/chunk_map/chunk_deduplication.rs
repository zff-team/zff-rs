...
impl ChunkMap for ChunkDeduplicationMap {
    ...
    fn is_full(&self) -> bool {
        if self.target_size < self.current_size() + 24 {
            //24 -> 8bytes for next chunk_no, 8 bytes for deduplication chunk_no, 8 bytes for the size of the encoded BTreeMap
            true
        } else {
            false
        }
    ...
    fn decrypt_and_decode<K, A, D>(
        key: K,
        encryption_algorithm: A,
        data: &mut D,
        chunk_no: u64,
    ) -> Result<Self>
    where
        ...
        Self: Sized,
    {
        ...
        let enc_buffer = Self::decrypt(
            key,
            inner_structure_data.structure_data,
            chunk_no,
            encryption_algorithm.borrow(),
        )?;
        ...
        Self: HeaderCoding,
    {
        ...
        let encrypted_map =
            Self::encrypt(key, encoded_map, chunk_no, encryption_algorithm.borrow())?;
        let encoded_version = Self::version().encode_directly();
        let encoded_object_number = self.object_number.encode_directly();
        let identifier = Self::identifier();
        let encoded_header_length = (DEFAULT_LENGTH_HEADER_IDENTIFIER
...
impl DeduplicationChunkMap {
    ...
    pub fn append_verification_hash<B: Borrow<Blake3Hash>>(
        &mut self,
        chunk_no: u64,
        verification_hash: B,
    ) -> Result<()> {
        match self {
            DeduplicationChunkMap::InMemory(map) => {
                map.verification_hash_map
                    .insert(chunk_no, *verification_hash.borrow());
                Ok(())
            }
            DeduplicationChunkMap::Redb(db) => {
                let write_txn = db.begin_write()?;
    ...
    pub fn get_chunk_number(&mut self, xxhash: u64) -> Result<HashSet<u64>> {
        match self {
            DeduplicationChunkMap::InMemory(map) => map
                .xxhash_map
                .get(&xxhash)
    ...
    pub fn get_verification_hash(&mut self, chunk_no: u64) -> Result<Option<Blake3Hash>> {
        match self {
            DeduplicationChunkMap::InMemory(map) => {
                Ok(map.verification_hash_map.get(&chunk_no).copied())
            }
            DeduplicationChunkMap::Redb(db) => {
                let read_txn = db.begin_read()?;
