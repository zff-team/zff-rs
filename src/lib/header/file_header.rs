...
impl FileHeader {
    ...
    pub fn new(
        ...
        metadata_ext: HashMap<String, MetadataExtendedValue>,
    ) -> FileHeader {
        Self {
            file_number,
            file_type,
    ...
    pub fn decode_at_encrypted_header_with_key<R, E>(
        data: &R,
        offset: u64,
        ...
        E: Borrow<EncryptionInformation>,
    {
        ...
        Self::decode_encrypted_header_with_key(&mut cursor, encryption_information)
    }

    /// decodes the encrypted header with the given key and [crate::header::EncryptionHeader].
    pub fn decode_encrypted_header_with_key<R, E>(
        data: &mut R,
        encryption_information: E,
    ) -> Result<Self>
    where
        ...
        E: Borrow<EncryptionInformation>,
    {
        ...
        let (file_type, filename, parent_file_number, metadata_ext) =
    ...
    fn decode_inner_content<R: Read>(
        ...
        let metadata_ext =
            HashMap::<String, MetadataExtendedValue>::decode_directly(inner_content)?;
        let inner_content = (file_type, filename, parent_file_number, metadata_ext);
        Ok(inner_content)
    }
}
...
impl Serialize for MetadataExtendedValue {
        ...
        S: Serializer,
    {
        match self {
            ...
            MetadataExtendedValue::PlatformString(value) => {
    ...
    fn decode_directly<R: Read>(data: &mut R) -> Result<Self> {
        ...
        match identifier {
            METADATA_EXT_TYPE_IDENTIFIER_U8 => {
                Ok(MetadataExtendedValue::U8(u8::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_U16 => {
                Ok(MetadataExtendedValue::U16(u16::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_U32 => {
                Ok(MetadataExtendedValue::U32(u32::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_U64 => {
                Ok(MetadataExtendedValue::U64(u64::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_I8 => {
                Ok(MetadataExtendedValue::I8(i8::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_I16 => {
                Ok(MetadataExtendedValue::I16(i16::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_I32 => {
                Ok(MetadataExtendedValue::I32(i32::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_I64 => {
                Ok(MetadataExtendedValue::I64(i64::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_STRING => Ok(MetadataExtendedValue::String(
                String::decode_directly(data)?,
            )),
            METADATA_EXT_TYPE_IDENTIFIER_HASHMAP => {
            ...
            METADATA_EXT_TYPE_IDENTIFIER_BYTEARRAY => Ok(MetadataExtendedValue::ByteArray(
                Vec::<u8>::decode_directly(data)?,
            )),
            METADATA_EXT_TYPE_IDENTIFIER_F32 => Ok(MetadataExtendedValue::F32(OrderedFloat::from(
                f32::decode_directly(data)?,
            ))),
            METADATA_EXT_TYPE_IDENTIFIER_F64 => Ok(MetadataExtendedValue::F64(OrderedFloat::from(
                f64::decode_directly(data)?,
            ))),
            METADATA_EXT_TYPE_IDENTIFIER_VEC => {
                let length = u64::decode_directly(data)?;
                let mut vec = Vec::new();
            ...
            METADATA_EXT_TYPE_IDENTIFIER_BOOL => {
                Ok(MetadataExtendedValue::Bool(bool::decode_directly(data)?))
            }
            METADATA_EXT_TYPE_IDENTIFIER_PLATFORM_STRING => Ok(
                MetadataExtendedValue::PlatformString(PlatformString::decode_directly(data)?),
            ),
            _ => Err(ZffError::new(
                ZffErrorKind::Unsupported,
