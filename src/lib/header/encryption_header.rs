...
use crate::{
    ...
    decrypt_pbkdf2sha256_aes256cbc, decrypt_scrypt_aes128cbc, decrypt_scrypt_aes256cbc,
};
...
impl TryFrom<ObjectHeader> for EncryptionInformation {
    ...
    fn try_from(obj_header: ObjectHeader) -> Result<Self> {
        match obj_header.encryption_header {
            ...
            Some(ref enc_header) => match enc_header.get_encryption_key() {
                None => Err(ZffError::new(
                    ZffErrorKind::EncryptionError,
                    ERROR_MISSING_ENCRYPTION_HEADER_KEY,
                )),
...
pub struct EncryptionHeader {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(
            serialize_with = "crate::helper::buffer_to_base64",
            deserialize_with = "crate::helper::base64_to_buffer"
        )
    )]
    ...
    #[cfg_attr(
        feature = "serde",
        serde(serialize_with = "crate::helper::option_buffer_to_base64")
    )]
