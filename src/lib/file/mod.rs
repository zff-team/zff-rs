    ...
    fn from(value: VirtualFileFooterContent) -> Self {
        match value {
            ...
            VirtualFileFooterContent::SpecialFile(rdev_id, stype) => {
                VirtualFileContent::SpecialFile(rdev_id, stype)
            }
            VirtualFileFooterContent::FileMap(seg, off) => {
                VirtualFileContent::FileMapPosition(seg, off)
            }
        }
    }
}
...
impl FileMetadata {
    ...
    pub(crate) fn vffc(&self) -> Result<&VirtualFileContent> {
        match &self.footer {
            FileFooterMetadata::FileFooter(_) => Err(ZffError::new(
                ZffErrorKind::Invalid,
                ERROR_ZFFREADER_MISSING_VFM,
            )),
            FileFooterMetadata::VirtualFileFooterMetadata(footer) => Ok(&footer.vfc),
        }
    }
