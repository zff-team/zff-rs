...
use crate::prelude::*;
use crate::{
    EncodingState, EncodingThreadPoolManager, PreparedChunk, Signature, chunking,
    io::{BufferedChunk, buffer_chunk},
};
...
impl FileEncoder {
    ...
    pub(crate) fn set_initial_chunk_number(&mut self, chunk_number: u64) {
        self.initial_chunk_number = chunk_number;
    ...
    pub(crate) fn get_next_chunk<D: ReadAt>(
        ...
        let buffered_chunk = match &mut self.filetype_encoding_information {
            ...
            FileTypeEncodingInformation::Symlink(symlink_real_path) => {
                let encoded_symlink_real_path =
                    PlatformString::from(symlink_real_path.as_os_str()).encode_directly();
        ...
        let encryption_algorithm = self
            .encryption_information
            .as_ref()
            .map(|encryption_information| &encryption_information.algorithm);
        let encryption_key = self
            .encryption_information
            .as_ref()
            .map(|encryption_information| &encryption_information.encryption_key);
    ...
    pub fn get_encoded_footer(&mut self) -> Result<Vec<u8>> {
        ...
        hashes_to_log(
            ...
            Some(self.file_header.file_number),
            &hash_values,
        );
