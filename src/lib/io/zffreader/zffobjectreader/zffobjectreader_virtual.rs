...
use crate::prelude::*;
use crate::{
    FileFooterMetadata, FileMetadata, VirtualFileContent,
    helper::{copy_chunk_content_to_buf, floor_btree_entry},
...
impl<R: ReadAt> ZffObjectReaderVirtual<R> {
    ...
    pub fn current_fileheader(&self) -> Result<FileHeader> {
        let metadata = self.files.get(&self.active_file).ok_or(ZffError::new(
    ...
    fn read_virtual_filemap(&self, filenumber: u64) -> Result<VirtualFileMap> {
        ...
        if let Ok(VirtualFileContent::FileMapPosition(segment_no, offset)) = filemetadata.vffc() {
            ...
            Err(ZffError::new(
                ZffErrorKind::Invalid,
                format!("{ERROR_ZFFREADER_MISSING_VFM}{filenumber}"),
            ))
        }
    }
    ...
    fn with_obj_metadata(object_no: u64, metadata: ArcZffReaderMetadata<R>) -> Result<Self> {
        ...
        for passive_object_number in &object_footer.passive_objects {
            ...
            let obj_metadata = obj_metadata.ok_or(ZffError::new(
                ZffErrorKind::Missing,
                format!("{ERROR_ZFFREADER_MISSING_PASSIVE_OBJECT}{passive_object_number}"),
            ))?;
        ...
        for (filenumber, header_segment_number) in &object_footer.file_header_segment_numbers {
            ...
            let filefooter = match metadata.segments.get(footer_segment_number) {
                ...
                Some(segment) => {
                    //check encryption
                    if let Some(enc_info) = &enc_info {
                        VirtualFileFooter::decode_at_encrypted_footer_with_key(
                            segment,
                            *footer_offset,
                            enc_info,
                        )?
        ...
        #[cfg(feature = "log")]
        debug!(
            "{} files were successfully initialized for virtual logical object {}.",
            files.len(),
    ...
    pub(crate) fn filemetadata_by_filenumber_mut(
        &mut self,
        filenumber: u64,
    ) -> Result<&mut FileMetadata> {
    ...
    fn cached_chunk(
        ...
        object_number: &u64,
        chunk_number: u64,
    ) -> std::io::Result<Arc<ChunkContent>> {
...
impl<R: ReadAt> ReadAtFile for ZffObjectReaderVirtual<R> {
    fn read_at_file(&self, buf: &mut [u8], offset: u64, file_no: u64) -> std::io::Result<usize> {
        let filemetadata = self.files.get(&file_no).ok_or(IoError::other(format!(
            "{ERROR_MISSING_FILE_NUMBER}{file_no}"
        )))?;
        ...
        let vffm = match &filemetadata.footer {
            ...
            FileFooterMetadata::FileFooter(_) => {
                return Err(IoError::other(ERROR_ZFFREADER_OPERATION));
            }
        };
        ...
        while read_bytes < bytes_to_read {
            ...
            let (extent_offset, vfe) = floor_btree_entry(&vfm.extents, virtual_pos).ok_or(
                IoError::other(format!("{ERROR_ZFFREADER_MISSING_VALUE_VFM}{virtual_pos}")),
            )?;

            if virtual_pos >= extent_offset + vfe.length {
                return Err(IoError::other(format!(
                    "{ERROR_ZFFREADER_MISSING_VALUE_VFM}{virtual_pos}"
                )));
            }
            ...
            let obj_metadata = self
                ...
                .ok_or(IoError::other(format!(
                    "{ERROR_ZFFREADER_MISSING_PASSIVE_OBJECT}{}",
            ...
            let chunk_content =
                self.cached_chunk(&vfe.source_object_number, current_chunk_number)?;
