...
use crate::prelude::*;
use crate::{
    FileMetadata, Segment, header::ObjectType as HeaderObjectType, helper::get_segment_of_chunk_no,
};
...
impl PreloadedChunkMaps {
    ...
    fn set_mode_redb(&mut self, mut db: Database) -> Result<()> {
        match self {
            PreloadedChunkMaps::None => (),
            PreloadedChunkMaps::InMemory(map) => {
                convert_in_memory_preloaded_chunkmaps_into_redb(&mut db, map)?
            }
...
impl<R: ReadAt> ZffReader<R> {
    ...
    pub fn list_decrypted_objects(&self) -> BTreeMap<u64, ObjectType> {
        ...
        for (k, v) in &self.object_reader {
            match v {
                ...
                ZffObjectReader::Physical(_) => {
                    map.insert(*k, ObjectType::Physical);
                }
                ZffObjectReader::Logical(_) => {
                    map.insert(*k, ObjectType::Logical);
                }
                ZffObjectReader::Virtual(_) => {
                    map.insert(*k, ObjectType::Virtual);
                }
            };
        }
        map
    ...
    pub fn preinitialize_virtual_object_filemaps(&mut self) -> Result<()> {
        if let Some(ZffObjectReader::Virtual(obj_reader)) =
            self.object_reader.get_mut(&self.active_object)
        {
    ...
    pub fn current_fileheader(&self) -> Result<FileHeader> {
        match self.object_reader.get(&self.active_object) {
            ...
            Some(ZffObjectReader::Physical(_)) => Err(ZffError::new(
                ZffErrorKind::Invalid,
                ERROR_ZFFREADER_OPERATION_PHYSICAL_OBJECT,
            )),
            Some(ZffObjectReader::Encrypted(_)) => Err(ZffError::new(
                ZffErrorKind::Invalid,
    ...
    pub fn current_filemetadata(&self) -> Result<&FileMetadata> {
        match self.object_reader.get(&self.active_object) {
            Some(ZffObjectReader::Logical(reader)) => Ok(reader.current_filemetadata()?),
            Some(ZffObjectReader::Physical(_)) => Err(ZffError::new(
                ZffErrorKind::Invalid,
                ERROR_ZFFREADER_OPERATION_PHYSICAL_OBJECT,
            )),
    ...
    pub fn filemetadata(&self, object_number: u64, file_number: u64) -> Result<&FileMetadata> {
        match self.object_reader.get(&object_number) {
            ...
            Some(ZffObjectReader::Virtual(reader)) => {
                reader.filemetadata_by_filenumber(file_number)
            }
            None => Err(ZffError::new(
                ZffErrorKind::Missing,
                format!("{ERROR_MISSING_OBJECT_NO}{}", self.active_object),
    ...
    pub fn chunk_data(&self, chunk_no: u64) -> Result<Vec<u8>> {
        let segment =
            match get_segment_of_chunk_no(chunk_no, &self.metadata.main_footer.chunk_header_maps) {
        ...
        let chunk_header =
            match extract_chunk_header_from_preloaded_chunkmap(&preloaded_chunkmaps, chunk_no) {
                Some(header) => header,
        ...
        if chunk_header.flags.encryption {
            return Err(ZffError::new(
                ZffErrorKind::Invalid,
                format!("{ERROR_CHUNK_IS_ENCRYPTED_}{chunk_no}"),
            ));
        }
        ...
        match segment.chunk_data(
            ...
            ChunkContent::SameBytes(single_byte) => {
                Ok(vec![single_byte; object_header.chunk_size as usize])
            }
        }
    }
    ...
    pub fn decrypt_object<P: AsRef<[u8]>>(
        &mut self,
        object_number: u64,
        decryption_password: P,
    ) -> Result<ObjectType> {
        ...
        let decrypted_reader = match object_reader {
            ZffObjectReader::Encrypted(reader) => {
                reader.decrypt_with_password(decryption_password)?
            }
            _ => {
                return Err(ZffError::new(
    ...
    /// If no chunkmap was initialized, a new in-memory map will be initialized by using this method.
    fn preload_chunk_samebytes_map(
        &mut self,
        chunk_number: u64,
        encryption_information: &Option<EncryptionInformation>,
    ) -> Result<()> {
    ...
    pub fn preload_chunk_samebytes_map_per_object(&mut self, object_number: u64) -> Result<()> {
        ...
        #[cfg(feature = "log")]
        log::debug!(
            "Preloading chunk samebytes map for object {}",
            object_number
        );
        let obj_reader = match self.object_reader.get(&object_number) {
            Some(reader) => reader,
    ...
    /// If no chunkmap was initialized, a new in-memory map will be initialized by using this method.
    fn preload_chunk_deduplication_map(
        &mut self,
        chunk_number: u64,
        encryption_information: &Option<EncryptionInformation>,
    ) -> Result<()> {
    ...
    pub fn preload_chunk_deduplication_map_per_object(&mut self, object_number: u64) -> Result<()> {
        ...
        #[cfg(feature = "log")]
        log::debug!(
            "Preloading chunk deduplication map for object {}",
            object_number
        );
        let obj_reader = match self.object_reader.get(&object_number) {
            Some(reader) => reader,
    ...
    fn unencrypted_object_no(&self) -> Vec<u64> {
        ...
        for (obj_no, reader) in &self.object_reader {
            if let ZffObjectReader::Encrypted(_) = reader {
                continue;
            };
    ...
    pub fn object_footer(&self, object_number: u64) -> Result<ObjectFooter> {
        self.object_reader
            ...
            .ok_or(ZffError::new(
                ZffErrorKind::Missing,
                format!("{ERROR_ZFFREADER_MISSING_OBJECT}{}", object_number),
            ))?
            .footer()
    }
...
fn try_find_footer<R: ReadAt>(reader: &mut R) -> Result<Footer> {
    ...
    if let Ok(main_footer) = MainFooter::decode_at(reader, footer_offset) {
        ...
        if let Ok(segment_footer) = SegmentFooter::decode_at(reader, footer_offset) {
            ...
            Err(ZffError::new(
                ZffErrorKind::EncodingError,
                ERROR_UNDECODABLE_SEGMENT_FOOTER,
            ))
        }
    } else {
        Err(ZffError::new(
            ZffErrorKind::EncodingError,
            ERROR_UNDECODABLE_MAIN_FOOTER,
        ))
    }
}
    ...
    R: ReadAt,
{
    ...
    let chunk_content = {
        ...
        let object_header_ref = &metadata
            .object_metadata
            .get(current_object_no)
            .ok_or(ZffError::new(
                ZffErrorKind::Missing,
                format!("{ERROR_MISSING_OBJECT_NO}{current_object_no}"),
            ))?
            .get()
            .ok_or(ZffError::new(
                ZffErrorKind::Missing,
...
fn initialize_object_reader_all<R: ReadAt>(
    metadata: ArcZffReaderMetadata<R>,
) -> Result<HashMap<u64, ZffObjectReader<R>>> {
    let mut obj_reader_map = HashMap::new();
    for obj_no in metadata.main_footer.object_footer.keys() {
        let obj_reader = initialize_object_reader(*obj_no, Arc::clone(&metadata))?;
...
fn initialize_unencrypted_object_reader<R: ReadAt>(
    ...
    metadata
        ...
        .ok_or_else(|| {
            ZffError::new(
                ZffErrorKind::Missing,
                format!("{ERROR_MISSING_OBJECT_NO}{obj_number}"),
            )
        })?
        .set(obj_metadata)
        .map_err(|_| {
            ZffError::new(
                ZffErrorKind::Invalid,
                format!("object already initialized: {obj_number}"),
            )
        })?;
...
fn initialize_encrypted_object_reader<R: ReadAt>(
    ...
    let obj_reader = ZffObjectReader::Encrypted(Box::new(ZffObjectReaderEncrypted::new(
        header,
        footer,
        Arc::clone(&metadata),
    )));
...
fn preloaded_redb_chunk_header_map_add_entry(
    db: &mut Database,
    chunk_no: u64,
...
fn preloaded_redb_chunk_samebytes_map_add_entry(
    db: &mut Database,
    chunk_no: u64,
    same_byte: u8,
) -> Result<()> {
...
fn preloaded_redb_chunk_deduplication_map_add_entry(
    db: &mut Database,
    chunk_no: u64,
    duplicated: u64,
) -> Result<()> {
...
fn extract_chunk_header_from_preloaded_chunkmap(
    preloaded_chunkmap: &PreloadedChunkMaps,
    chunk_number: u64,
) -> Option<ChunkHeader> {
...
fn extract_deduplication_chunks_from_preloaded_chunkmap(
    preloaded_chunkmap: &PreloadedChunkMaps,
    chunk_number: u64,
) -> Option<u64> {
    match preloaded_chunkmap {
        ...
        PreloadedChunkMaps::Redb(db) => {
            let read_txn = db.begin_read().ok()?;
            let table = read_txn
                .open_table(PRELOADED_CHUNK_DUPLICATION_MAP_TABLE)
                .ok()?;
...
fn extract_samebyte_from_preloaded_chunkmap(
    preloaded_chunkmap: &PreloadedChunkMaps,
    chunk_number: u64,
) -> Option<u8> {
    match preloaded_chunkmap {
        ...
        PreloadedChunkMaps::Redb(db) => {
            let read_txn = db.begin_read().ok()?;
            let table = read_txn
                .open_table(PRELOADED_CHUNK_SAME_BYTES_MAP_TABLE)
                .ok()?;
            let value = table.get(&chunk_number).ok()??.value();
            Some(value)
        }
...
fn get_chunks_of_unencrypted_object<R: ReadAt>(
    ...
    let chunk_numbers = match obj_reader {
        ZffObjectReader::Physical(reader) => {
            ...
            let last_chunk =
                reader.object_footer_unwrapped_ref().number_of_chunks + first_chunk - 1;
...
fn get_enc_info_from_obj_reader<R: ReadAt>(
    object_reader: &ZffObjectReader<R>,
) -> Result<Option<EncryptionInformation>> {
    let enc_info = match object_reader {
        ZffObjectReader::Physical(reader) => {
            EncryptionInformation::try_from(reader.object_header_ref())
        }
    ...
    R: ReadAt,
{
    let segment_no = match chunkmap_type {
        ...
        ChunkMapType::SamebytesMap => metadata
            .main_footer
            .chunk_samebytes_maps()
            .get(&chunk_number)?,
        ChunkMapType::DeduplicationMap => {
            metadata.main_footer.chunk_dedup_maps().get(&chunk_number)?
        }
    };
    let offset = match chunkmap_type {
        ChunkMapType::HeaderMap => metadata
            .segments
            .get(segment_no)?
            .footer()
        ...
        ChunkMapType::SamebytesMap => metadata
            ...
            .chunk_samebytes_map_table
            .get(&chunk_number),
        ChunkMapType::DeduplicationMap => metadata
            .segments
            .get(segment_no)?
            .footer()
