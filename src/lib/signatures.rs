...
use ed25519_dalek::{
    KEYPAIR_LENGTH, PUBLIC_KEY_LENGTH, SECRET_KEY_LENGTH, Signature as Ed25519Signature, Signer,
...
impl Signature {
    ...
    pub fn new_signingkey_from_base64<K: Into<String>>(key: K) -> Result<SigningKey> {
        ...
        if key.len() == KEYPAIR_LENGTH {
            ...
            Err(ZffError::new(
                ZffErrorKind::SigningError,
                ERROR_WRONG_SIGNATURE_KEY_LENGTH,
            ))
        }
    }
    ...
    /// ```
    pub fn verify_with_base64_key<K: Into<String>>(
        key: K,
        message: &[u8],
        signature: [u8; ED25519_DALEK_SIGNATURE_LEN],
    ) -> Result<bool> {
        let key = base64engine.decode(key.into())?;
        ...
        K: AsRef<[u8]>,
    {
        ...
        let verifying_key = if key.len() == KEYPAIR_LENGTH {
            // if the key is a signing key
            let mut key_slice = [0u8; KEYPAIR_LENGTH];
            key_slice.copy_from_slice(key);
